...
var LayaUISample=(function(){
	...
	__proto._initGame=function(){
		this.gmv=new GameView();
		Laya.stage.addChild(this.gmv);
	}
...
var GameMap=(function(){
	...
	__class(GameMap,'src.game.GameMap');
	var __proto=GameMap.prototype;
	__proto.setMap=function(){
		this.mapAry=[];
		for (var i=0;i < this.glb.row;i++){
			if(!this.mapAry[i])
				this.mapAry[i]=[]
			for (var j=0;j < this.glb.col;j++){
				this.mapAry[i][j]=this.getRandom;
			}
		}
		if(this.glb.isDebug){
			console.log("=====================地图数据=====================")
			for (i=0;i < this.mapAry.length;i++){
				console.log(this.mapAry[i]);
			}
		}
		this.glb.mapAry=this.mapAry;
	}

	__getset(0,__proto,'getRandom',function(){
...
var Global=(function(){
	function Global(){
		...
		/**地图数据*/
		this.mapAry=null;
		this.showW=this.iwidth *this.col;
		this.showH=this.iheight *this.row;
	}

	__class(Global,'src.game.Global');
	Global.Ins=function(){
		return Global._in=Global._in|| new Global();
	}
...
//class src.game.GameView extends laya.display.Sprite
var GameView=(function(_super){
	function GameView(){
		...
		this.glb=Global.Ins();
		this.startPo=new Point((this.glb.sceneW-this.glb.showW)>> 1,(this.glb.sceneH-this.glb.showH)>> 1);
		this._initGame();
	}

	__class(GameView,'src.game.GameView',_super);
	var __proto=GameView.prototype;
	__proto._initGame=function(){
		this.isPlay=false;
		this.waterNum=this.glb.curWater;
		this.level=this.glb.curLevel;
		this.initDataTxt();
		this.resetMap();
		this.drawLines();
		this.drawCell();
		this.mapAry=this.glb.mapAry;
	...
	__proto.keyDown=function(e){
		if(e.keyCode==32){
			if(!this.scneAni){
				this.scneAni=new SceneAni();
				this.scneAni.pos(this.startPo.x,this.startPo.y);
			}
			this.addChild(this.scneAni);
			this.scneAni.showSceneAni();
		}
	}

	__proto.drawCell=function(){
		for (var i=0;i < this.glb.row;i++){
			if(!this.txtAry[i])
				this.txtAry[i]=[];
			for (var j=0;j < this.glb.col;j++){
				if(!this.txtAry[i][j]){
					var cell=new Water();
					this.txtAry[i][j]=cell;
				}
				else
				cell=this.txtAry[i][j];
				cell.setData(this.glb.mapAry[i][j]);
				cell.pos(this.startPo.x+j*this.glb.iwidth,this.startPo.y+i*this.glb.iheight);
				this.addChild(cell);
				cell.on("click",this,this.onClickCell,[i,j]);
			}
		}
	}

	__proto.updateWateNum=function(add){
		(add===void 0)&& (add=false);
		if(add)
			++this.waterNum;
		else
		--this.waterNum;
		this.waterTxt.text=this.waterNum.toString();
	}

	__proto.updateLevel=function(){
		this.isPlay=true;
		++this.level;
		this.glb.curLevel=this.level;
		this.levelTxt.text=this.level.toString();
		if(!this.scneAni){
			this.scneAni=new SceneAni();
			this.scneAni.pos(this.startPo.x,this.startPo.y);
		}
		this.addChild(this.scneAni);
		this.scneAni.showSceneAni();
		this.scneAni.on("ENTER_NEXT_PASS",this,this.moveCom);
	}

	__proto.moveCom=function(){
		this.isPlay=false;
		this.gmp.setMap();
		this.drawCell();
		this.mapAry=this.glb.mapAry;
	...
	__proto.onClickCell=function(row,col,eve,dir){
		(dir===void 0)&& (dir="");
		var tempAry=[];
		if(eve){
			if(this.isPlay)
			...
			this.updateWateNum(false);
		}
		else
		target=this.txtAry[row][col];
		var isPass=true;
		var isBomb=false;
		var num=this.mapAry[row][col]+1;
		if(num <=4){
			if(num==1){
				if(eve)
					isPass=false;
		...
		this.mapAry[row][col]=num;
		target.setData(num);
		if(isPass){
			if(isBomb){
				this.addExtWater();
				Laya.timer.once(300,this,this.checkBomb,[row,col,tempAry,true]);
				this.playAnyAni(target,row,col);
			}
			else{
				Laya.timer.once(300,this,this.checkOfFission,[row,col,dir],false);
				this.starPlayAni(target,dir);
			}
		}
	}

	__proto.addExtWater=function(){
		++this.curScore;
		if(this.curScore >=3){
			this.curScore=this.curScore-3;
			this.updateWateNum(true);
		}
	}

	__proto.playAnyAni=function(target,row,col){
		if(this.mapAry[row-1] && this.mapAry[row-1][col] !=undefined)
			this.starPlayAni(target,"up");
		if(this.mapAry[row+1] && this.mapAry[row+1][col] !=undefined)
			this.starPlayAni(target,"down");
		if(this.mapAry[row] && this.mapAry[row][col-1] !=undefined)
			this.starPlayAni(target,"left");
		if(this.mapAry[row] && this.mapAry[row][col+1] !=undefined)
			this.starPlayAni(target,"right");
	}

	__proto.starPlayAni=function(target,dir){
		++this.curPoint;
		target.playOneAni(dir);
		target.on("TWC_COMPLETE",this,this.tweenCom);
	}

	__proto.tweenCom=function(){
		--this.curPoint;
		Laya.timer.once(100,this,this.statePlay);
	}

	__proto.statePlay=function(){
		this.isPlay=this.curPoint > 0;
		if(!this.isPlay && this.checkOver()){
			this.updateLevel();
		}
	}

	__proto.checkOver=function(){
		for (var i=0;i < this.mapAry.length;i++){
			for (var j=0;j < this.mapAry[i].length;j++){
				if(this.mapAry[i][j] > 0)
					return false;
			}
		}
		return true;
	...
	__proto.checkOfFission=function(row,col,dir){
		switch(dir){
			case "up":{
					if(this.mapAry[row-1] && this.mapAry[row-1][col] !=undefined){
						this.onClickCell(row-1,col,null,"up");
					}
					break ;
				}
			case "down":{
					if(this.mapAry[row+1] && this.mapAry[row+1][col] !=undefined){
						this.onClickCell(row+1,col,null,"down");
					}
					break ;
				}
			case "left":{
					if(this.mapAry[row] && this.mapAry[row][col-1] !=undefined){
						this.onClickCell(row,col-1,null,"left");
					}
					break ;
				}
			default :{
					if(this.mapAry[row] && this.mapAry[row][col+1] !=undefined){
						this.onClickCell(row,col+1,null,"right");
					}
					break ;
				}
			}
	...
	__proto.checkBomb=function(row,col,ary){
		if(this.mapAry[row-1] && this.mapAry[row-1][col] !=undefined){
			this.onClickCell(row-1,col,null,"up");
		}
		if(this.mapAry[row+1] && this.mapAry[row+1][col] !=undefined){
	...
	__proto.drawLines=function(){
		var lineSp=new Sprite();
		lineSp.pos(this.startPo.x,this.startPo.y);
		this.addChild(lineSp);
		for (var i=0;i < this.glb.row+1;i++){
			lineSp.graphics.drawLine(0,i *this.glb.iheight,this.glb.col *this.glb.iwidth,i *this.glb.iheight,"0x000000");
			lineSp.graphics.drawLine(i *this.glb.iwidth,0,i *this.glb.iwidth,this.glb.row *this.glb.iheight,"0x000000");
		}
	}

	__proto.resetMap=function(){
		this.gmp=new GameMap();
		this.gmp.setMap();
	}

	__proto.initDataTxt=function(){
		var numTxt=new Text();
		numTxt.text="水滴：";
		numTxt.color="#38a4ec";
		numTxt.fontSize=23;
		numTxt.pos(this.startPo.x,this.startPo.y-30);
		this.addChild(numTxt);
		this.waterTxt=new Text();
		this.waterTxt.text=this.waterNum.toString();
		this.waterTxt.color="#38a4ec";
		this.waterTxt.fontSize=23;
		this.addChild(this.waterTxt);
		this.waterTxt.pos(this.startPo.x+numTxt.width,this.startPo.y-30);
		var lvTxt=new Text();
		lvTxt.text="等级：";
		lvTxt.color="#3dc631";
		lvTxt.fontSize=23;
		this.addChild(lvTxt);
		lvTxt.pos(this.startPo.x+260,this.startPo.y-30);
		this.levelTxt=new Text();
		this.levelTxt.text=this.level.toString();
		this.levelTxt.color="#3dc631";
		this.levelTxt.fontSize=23;
		this.addChild(this.levelTxt);
		this.levelTxt.pos(lvTxt.x+lvTxt.width,this.startPo.y-30);
	}
...
//class src.game.SceneAni extends laya.display.Sprite
var SceneAni=(function(_super){
	function SceneAni(){
		this.ani=null;
		this.lvTxt=null;
		this.ggTxt=null;
		this.spCt=null;
		this.isCom=false;
		SceneAni.__super.call(this);
		this.mouseEnabled=true;
		this.size(Global.Ins().showW,Global.Ins().showH);
		this.ani=new Sprite();
		this.addChild(this.ani);
		this.ani.visible=false;
		this.ani.graphics.drawRect(0,0,Global.Ins().showW,1,"#000000");
		this.spCt=new Sprite();
		this.addChild(this.spCt);
		this.spCt.y=-50;
		this.spCt.visible=false;
		this.ggTxt=new Text();
		this.ggTxt.color='#FF0000';
		this.ggTxt.fontSize=30;
		this.ggTxt.stroke=2;
		this.ggTxt.strokeColor='#FFFFFF';
		this.ggTxt.bold=true;
		this.ggTxt.text="真棒！继续挑战吧";
		this.spCt.addChild(this.ggTxt);
		this.ggTxt.pos((Global.Ins().showW-this.ggTxt.width)>> 1,40);
		this.lvTxt=new Text();
		this.lvTxt.color='#FF0000';
		this.lvTxt.fontSize=30;
		this.lvTxt.stroke=2;
		this.lvTxt.strokeColor='#FFFFFF';
		this.lvTxt.bold=true;
		this.lvTxt.text="";
		this.spCt.addChild(this.lvTxt);
		this.on("click",this,this.enterNext);
	}

	__class(SceneAni,'src.game.SceneAni',_super);
	var __proto=SceneAni.prototype;
	__proto.showSceneAni=function(){
		this.isCom=false
		this.ani.visible=true;
		this.lvTxt.text="LV."+Global.Ins().curLevel;
		Laya.timer.loop(20,this,this.addHandler);
	}

	__proto.addHandler=function(){
		this.ani.graphics.clear();
		this.ani.height+=5;
		var num=this.ani.height > Global.Ins().showH ? Global.Ins().showH :this.ani.height;
		this.ani.graphics.drawRect(0,0,Global.Ins().showW,num,"#000000");
		this.ani.alpha=0.5;
		if(this.ani.height >=Global.Ins().showH+20){
			this.lvTxt.pos((Global.Ins().showW-this.lvTxt.width)>> 1,0);
			this.spCt.visible=true;
			Laya.timer.clear(this,this.addHandler);
			Tween.to(this.spCt,{y:Global.Ins().showH >> 1},700,Ease.backOut,Handler.create(this,this.lvTxtCom));
		}
	}

	__proto.lvTxtCom=function(){
		this.isCom=true
	}

	__proto.enterNext=function(){
		if(this.isCom){
			this.hideAni();
			this.event("ENTER_NEXT_PASS");
		}
	}

	__proto.hideAni=function(){
		this.spCt.visible=false;
		this.spCt.y=-50;
		this.ani.visible=false;
		this.ani.height=1;
...
//class src.game.Water extends laya.display.Sprite
var Water=(function(_super){
	function Water(){
		...
		this.iwidth=Global.Ins().iwidth;
		this.iheight=Global.Ins().iheight
		this.numTxt=new Text();
		this.numTxt.mouseEnabled=true;
		this.numTxt.width=this.iwidth;
		this.numTxt.height=this.iheight;
		this.numTxt.align="center";
		this.numTxt.valign="middle";
		this.numTxt.color="#ff0000";
		this.numTxt.fontSize=22;
		this.addChild(this.numTxt);
		this.aniAry=[]
		for (var i=0;i < 4;i++){
			var aniSp=new Sprite();
			aniSp.graphics.drawCircle(0,0,5,"#ff0000");
			aniSp.pos((this.iwidth+2.5)/2,(this.iheight+2.5)/2);
			aniSp.visible=false;
			this.addChild(aniSp);
			this.aniAry.push(aniSp);
		}
	}

	__class(Water,'src.game.Water',_super);
	...
	__proto.playOneAni=function(dir){
		var ty=(this.iheight+2.5)/2;
		var tx=(this.iwidth+2.5)/2;
		var target;
		if(dir=="up"){
			ty=ty-this.iheight;
			target=this.aniAry[0];
		}
		else if(dir=="down"){
			ty=ty+this.iheight;
			target=this.aniAry[1];
		}
		else if(dir=="left"){
			tx=tx-this.iwidth;
			target=this.aniAry[2];
		}
		else if(dir=="right"){
			tx=tx+this.iwidth;
			target=this.aniAry[3];
		}
		target.visible=true;
		Tween.to(target,{x:tx,y:ty},300,null,Handler.create(this,this.tcwHandler,[target],false));
	}

	__proto.tcwHandler=function(target){
		target.visible=false;
		target.pos((this.iwidth+2.5)/2,(this.iheight+2.5)/2);
		this.event("TWC_COMPLETE")
	}
